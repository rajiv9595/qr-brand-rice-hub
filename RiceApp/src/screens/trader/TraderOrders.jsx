...
export default function TraderOrders() {
  ...
  const handleUpdateStatus = async (id, status) => {
    try {
      ...
      const statusLabels = {
        'Confirmed': 'అంగీకరించబడింది (Accepted)',
        'Shipped': 'డిస్పాచ్ చేయబడింది (Dispatched)',
        'Delivered': 'డెలివరీ చేయబడింది (Delivered)',
        'Cancelled': 'రద్దు చేయబడింది (Cancelled)'
      };
      
      const statusMsg = statusLabels[status] || status;
      Alert.alert('విజయం (Success)', `ఆర్డర్ ${statusMsg}.`);
    } catch (err) {
